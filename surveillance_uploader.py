"""
Surveillance Images Uploader

This script uploads surveillance images from data/professional_images/surveillance/
...
class SurveillanceUploader:
    """Surveillance Images Uploader for Azure Blob Storage and Cosmos DB"""
    
    def __init__(self):
        """Initialize the uploader with Azure clients"""
        self.category = "surveillance"
        self.source_folder = "data/professional_images/surveillance"
        ...
        logger.info(f"SurveillanceUploader initialized for category: {self.category}")
    ...
    def get_image_files(self) -> List[Path]:
        """Get list of image files in the surveillance folder"""
    ...
    def upload_all_images(self) -> Dict[str, Any]:
        """Upload all images in the surveillance folder"""
        logger.info("Starting bulk upload of all surveillance images...")
    ...
    def upload_selected_images(self, selected_names: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Starting upload of selected surveillance images: {selected_names}")
    ...
    def delete_all_images(self) -> Dict[str, Any]:
        ...
        logger.info("Starting bulk deletion of all surveillance images...")
        
        # Get all blobs in the container with surveillance prefix
        try:
    ...
    def delete_selected_images(self, selected_names: List[str]) -> Dict[str, Any]:
        ...
        logger.info(f"Starting deletion of selected surveillance images: {selected_names}")
    ...
    def view_cosmos_images(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Note: Legacy documents are healthcare-specific (IM-* pattern) so don't include them for surveillance
            ...
            for item in items:
                cosmos_data = {
                    ...
                    "surveillance_metadata": item.get("surveillance_metadata", {}),
                    "document_type": "current"
                }
                cosmos_info.append(cosmos_data)
            
            logger.info(f"Found {len(items)} current surveillance images in Cosmos DB")
            return cosmos_info
    ...
    def count_cosmos_images(self) -> int:
        ...
        try:
            ...
            logger.info(f"Found {count} surveillance images in Cosmos DB")
            return count
    ...
    def view_detailed_images(self) -> Dict[str, Any]:
        ...
        if cosmos_images:
            ...
            for i, img in enumerate(cosmos_images, 1):
                ...
                # Show surveillance-specific metadata
                if img['surveillance_metadata']:
                    surv_meta = img['surveillance_metadata']
                    print(f"       Camera: {surv_meta.get('camera_type', 'N/A')} | Location: {surv_meta.get('location', 'N/A')}")
...
def main():
    """Main function to run the surveillance uploader"""
    try:
        uploader = SurveillanceUploader()
        
        print("📹 Surveillance Images Uploader")
        print("=" * 50)
        print("Available options:")
        print("1. Upload all surveillance images")
        ...
        while True:
            ...
            if choice == "1":
                print("\nUploading all surveillance images...")
                ...
                if not image_files:
                    print("❌ No images found in surveillance folder")
                    continue
                ...
                confirm = input("\n⚠️  Are you sure you want to delete ALL surveillance images? (yes/no): ").strip().lower()
                if confirm == 'yes':
                    print("\n🗑️  Deleting all surveillance images...")
